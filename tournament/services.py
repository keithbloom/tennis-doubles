...
from django.db.models import Q, Prefetch, F, Value, CharField, Case, When, IntegerField, Sum
...
class MatchResultService:
    ...
    def get_match_result(self, match: Match) -> Dict[str, Any]:
    ...
    def _get_sets(self, match: Match) -> List[tuple]:
        ...
        sets = [(match.set1_team1, match.set1_team2), 
    ...
    def _determine_winner(self, sets_won: Dict[str, int], match: Match) -> Optional[Team]:
...
class StandingsCalculator:
    ...
    def calculate_standings(self, tournament_group: TournamentGroup) -> List[Dict[str, Any]]:
...
class TournamentGridBuilder:
    ...
    def _build_group_data(self, tournament_group: TournamentGroup, 
                          tournament: Tournament) -> Dict[str, Any]:
        ...
        return {
            ...
            'standings': self.standings_calculator.calculate_standings(tournament_group)
        }
    
    def _build_match_grid(self, teams: List[Team], tournament: Tournament) -> List[List]:
    ...
    def _get_grid_cell_value(self, team1: Team, team2: Team, 
    ...
    def _get_annotated_matches(self, tournament_group: TournamentGroup, 
                              tournament: Tournament):
        """Get matches with annotations for display"""
        return Match.objects.filter(
            tournament=tournament,
            team1__tournament_group=tournament_group
        ).order_by('-date_played').annotate(
            team1_name=Concat(
            ...
            match_winner=self._get_match_winner_annotation(tournament)
        ).values(
            "id", "team1_name", "team2_name",
            "set1_team1", "set1_team2", "set2_team1", "set2_team2",
            "set3_team1", "set3_team2", "set1_winner", "set2_winner",
            "set3_winner", "match_winner", "date_played"
        )
    
    def _get_match_winner_annotation(self, tournament):  # Or wherever you're using this
        return (
            Match.objects
